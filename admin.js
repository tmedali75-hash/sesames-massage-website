  ...
  function renderStatsAdmin() {
    ...
    wrap.innerHTML = DATA.stats
      ...
      <div class="admin-row" style="grid-template-columns:1fr 90px 1fr;">
        <input type="text" value="${s.label}" data-i="${i}" data-f="label" class="js-stat-field" placeholder="Libellé">
        <input type="number" value="${s.valeur}" data-i="${i}" data-f="valeur" class="js-stat-field" placeholder="Valeur">
        <input type="text" value="${s.suffixe || ""}" data-i="${i}" data-f="suffixe" class="js-stat-field" placeholder="Suffixe (+, %, ...)">
      </div>`
      )
      .join("");
  ...
  function renderHoraires() {
    ...
    wrap.innerHTML = DATA.centre.horaires
      .map(
        (h, i) => `
      <div class="admin-row">
        <input type="text" value="${h.jour}" data-i="${i}" data-f="jour" class="js-horaire">
        <input type="text" value="${h.heures}" data-i="${i}" data-f="heures" class="js-horaire">
  ...
  function serviceRow(s) {
    ...
    <div class="admin-card" data-id="${s.id}">
      <div class="admin-card-grid">
        ...
        <input type="text" class="js-s-nom" value="${s.nom}" placeholder="Nom du service">
        <input type="number" class="js-s-duree" value="${s.duree}" placeholder="Durée (min)">
        <input type="number" class="js-s-prix" value="${s.prix}" placeholder="Prix (DT)">
      </div>
      <textarea class="js-s-desc" rows="2" placeholder="Description">${s.description}</textarea>
      <input type="text" class="js-s-image" value="${s.image}" placeholder="URL de l'image">
  ...
  function renderGalerie() {
    ...
    wrap.innerHTML = DATA.galerie
      .map(
        (src, i) => `
      <div class="admin-row">
        <img src="${src}" class="admin-thumb">
        <input type="text" value="${src}" data-i="${i}" class="js-gal-url">
  ...
  function renderTemoignages() {
    ...
    wrap.innerHTML = DATA.temoignages
      ...
      <div class="admin-card">
        <div class="admin-card-grid">
          <input type="text" value="${t.nom}" data-i="${i}" data-f="nom" class="js-t-field" placeholder="Nom">
          <select data-i="${i}" data-f="note" class="js-t-field">
            ${[5, 4, 3, 2, 1].map(n => `<option value="${n}" ${t.note === n ? "selected" : ""}>${n} étoiles</option>`).join("")}
          </select>
        </div>
        <textarea data-i="${i}" data-f="texte" class="js-t-field" rows="2">${t.texte}</textarea>
        <div class="admin-card-actions">
  ...
  function renderReservations() {
    ...
    wrap.innerHTML = list
      ...
      <div class="admin-card">
        ...
        ${r.message ? `<p style="margin:6px 0 0;color:var(--ink-soft);font-size:.88rem;">${r.message}</p>` : ""}
        <div class="admin-card-actions">
          <button class="btn btn-sm admin-danger js-resa-del" data-id="${r.id}">Supprimer</button>
        </div>
      </div>`
      )
      .join("");
