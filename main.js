...
(function () {
  ...
  function renderCentre() {
    ...
    const waNum = (DATA.centre.whatsapp || "").replace(/\D/g, "");
    $$(".js-wa-link").forEach(el => {
      if (!waNum) {
        // Pas de numéro WhatsApp configuré : on masque le bouton plutôt que
        // de proposer un lien cassé (https://wa.me/?text=...).
    ...
    if (reseauxEl && DATA.centre.reseaux) {
      const links = [];
      if (DATA.centre.reseaux.facebook) links.push(`<a href="${DATA.centre.reseaux.facebook}" target="_blank" rel="noopener noreferrer">Facebook</a>`);
      if (DATA.centre.reseaux.instagram) links.push(`<a href="${DATA.centre.reseaux.instagram}" target="_blank" rel="noopener noreferrer">Instagram</a>`);
  ...
  function bindReservationForm() {
    ...
    // Verrou anti double-soumission : sans lui, un double-clic ou un
    // double-tap (fréquent sur mobile) déclenche deux soumissions. La
    // première réussit et réinitialise le formulaire (form.reset()) ;
    // la seconde se retrouve alors avec des champs vides, échoue la
    // validation et affiche un message d'erreur juste après le message
    // de succès — donnant l'impression que la réservation "a planté"
    // alors qu'elle avait en réalité bien été enregistrée.
    ...
    form.addEventListener("submit", e => {
      ...
      const saved = SesamesStore.save(DATA);

      if (saved) {
        showMsg(msgEl, "Votre demande a bien été envoyée. Nous vous confirmons rapidement votre créneau !", true);
        form.reset();
      } else {
        // Le stockage local est indisponible (navigation privée stricte,
        // aperçu en bac à sable, etc.) : on informe honnêtement la
        // personne plutôt que d'afficher un faux message de succès.
        DATA.reservations.pop();
        showMsg(msgEl, "Impossible d'enregistrer la demande dans ce navigateur. Merci de nous contacter directement par téléphone ou WhatsApp pour réserver.", false);
      }

      setTimeout(() => {
        if (saved) closeReservation();
        isSubmitting = false;
        if (submitBtn) {
